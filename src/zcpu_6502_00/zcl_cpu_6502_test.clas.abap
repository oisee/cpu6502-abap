CLASS zcl_cpu_6502_test DEFINITION
  PUBLIC
  CREATE PUBLIC
  FOR TESTING
  DURATION SHORT
  ...
  PRIVATE SECTION.
    ...
    METHODS test_msbasic FOR TESTING.

ENDCLASS.



CLASS ZCL_CPU_6502_TEST IMPLEMENTATION.


  METHOD test_msbasic.
    " Test MS-BASIC using the speedrun class
    DATA(ls_result) = zcl_cpu_6502_basic_test=>run_test( ).

    IF ls_result-error_message IS NOT INITIAL.
      cl_abap_unit_assert=>fail( msg = ls_result-error_message ).
    ENDIF.

    " Get log for diagnostic output
    DATA(lt_log) = zcl_cpu_6502_basic_test=>get_log( ).
    DATA lv_log_text TYPE string.
    LOOP AT lt_log INTO DATA(ls_log).
      lv_log_text = lv_log_text && |[{ ls_log-role }] { ls_log-text } / |.
    ENDLOOP.

    cl_abap_unit_assert=>assert_equals(
      act = ls_result-success
      exp = abap_true
      msg = |{ ls_result-assertions_pass }/{ ls_result-assertions_total } - Log: { lv_log_text }| ).
  ENDMETHOD.
  ...
  METHOD load_program.
    ...
    mo_bus->zif_cpu_6502_bus~load( iv_addr = iv_addr iv_data = iv_program ).
    lv_addr_lo = iv_addr MOD 256.
    lv_addr_hi = iv_addr / 256.
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 65532 iv_val = lv_addr_lo ).
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 65533 iv_val = lv_addr_hi ).
  ...
  METHOD test_sta_zp.
    ...
    mo_cpu->step( ).
    mo_cpu->step( ).
    DATA(lv_val) = mo_bus->zif_cpu_6502_bus~read( 16 ).
  ...
  METHOD test_sta_abs.
    ...
    mo_cpu->step( ).
    mo_cpu->step( ).
    DATA(lv_val) = mo_bus->zif_cpu_6502_bus~read( 8192 ).
  ...
  METHOD test_inc_zp.
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 16 iv_val = 255 ).
  ...
  METHOD test_dec_zp.
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 16 iv_val = 0 ).
  ...
  METHOD test_jmp_ind.
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 512 iv_val = 16 ).
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 513 iv_val = 128 ).
  ...
  METHOD test_zp_indexed.
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 18 iv_val = 99 ).
  ...
  METHOD test_abs_indexed.
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 8195 iv_val = 77 ).
  ...
  METHOD test_indirect_x.
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 18 iv_val = 0 ).
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 19 iv_val = 32 ).
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 8192 iv_val = 123 ).
  ...
  METHOD test_indirect_y.
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 16 iv_val = 0 ).
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 17 iv_val = 32 ).
    mo_bus->zif_cpu_6502_bus~write( iv_addr = 8197 iv_val = 88 ).
